...
from guti.noise_models import NOISE_MODELS, _US_TRANSMIT_PRESSURE  # noqa: E402

README_PATH = REPO_ROOT / "README.md"
TABLE_BEGIN = "<!-- BEGIN GENERATED MODALITY NOISE LEVELS -->"
TABLE_END = "<!-- END GENERATED MODALITY NOISE LEVELS -->"

# Display name, formatting style, base unit, and a scale factor that converts the
# stored value into the displayed unit, per modality, in order.  Each entry's
# first field is a tuple of candidate keys (the TD-fNIRS model has been
# registered under both "td_fnirs" and the older "td_fnirs_analytical").
#   style "si"    -> SI-prefixed physical unit (e.g. 50 fT, 92.5 nV, 20.8 µPa)
#   style "ratio" -> dimensionless, shown as % / ppm / ppb
# The ultrasound forward model stores pressure as a ratio to the transmit
# pressure, so we multiply by it to recover absolute Pascals (SNR is preserved).
ROWS = [
    (("eeg",), "EEG", "si", "V", 1.0),
    (("meg_opm",), "MEG (OPM)", "si", "T", 1.0),
    (("meg_squid",), "MEG (SQUID)", "si", "T", 1.0),
    (("cw_fnirs",), "CW-fNIRS", "ratio", "ΔI/I", 1.0),
    (("td_fnirs", "td_fnirs_analytical"), "TD-fNIRS", "ratio", "ΔI/I", 1.0),
    (("us_analytical",), "Ultrasound", "si", "Pa", _US_TRANSMIT_PRESSURE),
    (("fmri_bold",), "fMRI (BOLD)", "ratio", "fractional BOLD", 1.0),
]
...
def build_table() -> str:
    ...
    lines = [header]
    for candidates, name, style, unit, scale in ROWS:
        m = resolve_model(candidates)
        values = (
            m.today_best_noise * scale,
            m.physical_floor_noise * scale,
            m.typical_signal_amplitude * scale,
        )
        if style == "si":
            cells = [_si_format(v, unit) for v in values]
        elif style == "ratio":
            label, ratio_scale = _ratio_unit(values[2])
            cells = [_ratio_format(v, label, ratio_scale) for v in values]
        else:
...
def write_into_readme(table: str) -> None:
    ...
    block = (
        ...
        "intensity ratios ΔI/I, and fMRI is fractional BOLD signal. Ultrasound "
        "is shown in Pascals (the forward-model pressure ratio scaled by the "
        "~10 kPa transmit pressure).\n\n"
        f"{TABLE_END}"
    )
