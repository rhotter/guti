"""Plot SV spectra for the four TD-fNIRS analytical sweeps.

Saves one figure per sweep under guti/modalities/td_fnirs/results/.
"""
import sys
...
PIN_GRID_RES_MM = 3.0   # converged grid
PIN_MAX_DIST = 40.0
PIN_N_TIME_GATES = 6
...
SWEEPS = [
    dict(
        ...
        title="TD-fNIRS — num_sensors sweep",
    ...
    dict(
        ...
        title="TD-fNIRS — grid_resolution_mm sweep",
    ...
    dict(
        ...
        title="TD-fNIRS — max_dist sweep",
        fname="svd_spectrum_max_dist.png",
    ),
    dict(
        param_key="n_time_gates",
        constant_params=Parameters(
            num_sensors=PIN_NUM_SENSORS,
            grid_resolution_mm=PIN_GRID_RES_MM,
            max_dist=PIN_MAX_DIST,
        ),
        title="TD-fNIRS — n_time_gates sweep",
        fname="svd_spectrum_n_time_gates.png",
    ),
]
...
def plot_sweep(param_key, constant_params, title, fname,
    ...
    # blank the whole figure (e.g. n_time_gates=1 can produce NaN singular values).
