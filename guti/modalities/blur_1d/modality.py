...
class Blur1D(ImagingModality):
    def __init__(self, params: Optional[Parameters] = None):
        """
        Relevant params:
        - input_dim: number of input grid points
        - output_dim: number of output grid points
        """
    ...
    def setup_geometry(self) -> None:
        self.x_in = np.linspace(0, self.L, self.params.input_dim)
        self.x_out = np.linspace(0, self.L, self.params.output_dim)

    def compute_forward_model(self) -> np.ndarray:
        dx = self.L / self.params.input_dim

        # Create convolution kernel matrix
        K = np.zeros((self.params.output_dim, self.params.input_dim))
        for i in range(self.params.output_dim):
            for j in range(self.params.input_dim):
                # Gaussian kernel
